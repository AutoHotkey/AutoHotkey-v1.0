...
// savings will grow larger if more complexity is ever added to the hooks.
{
	...
	key_type &this_key = *((aHook == g_KeybdHook && ksc[aSC].sc_takes_precedence) ? (ksc + aSC) : (kvk + aVK));
	...
	if (pPrefixKey && pPrefixKey != &this_key && !aKeyUp)
	...
	if (this_key.used_as_prefix && !aKeyUp && (!pPrefixKey || !this_key.used_as_suffix || &this_key == pPrefixKey))
	{
		...
		{
			// Check hook type too in case a script every explicitly specifies scan code zero as a hotkey:
			hotkey_id_with_flags = (aHook == g_KeybdHook && ksc[aSC].sc_takes_precedence)
...
// on "send a down event to make up" to find that section.
	if (   aKeyUp && (aVK && (kvk[aVK].no_suppress & NO_SUPPRESS_PREFIX))
		|| (aSC && (ksc[aSC].no_suppress & NO_SUPPRESS_PREFIX))   )
		no_suppress = true;
	...
	if (this_key.as_modifiersLR && aKeyUp)
...
// The caller has already ensured that vk and/or sc is a modifier such as VK_CONTROL.
{
	...
	if (aVK)
	{
		...
		case VK_CONTROL:
			...
			{
			case VK_MENU:
				kvk[VK_MENU].used_as_prefix = true;
				kvk[VK_LMENU].used_as_prefix = true;
				kvk[VK_RMENU].used_as_prefix = true;
				ksc[SC_LALT].used_as_prefix = true;
				ksc[SC_RALT].used_as_prefix = true;
				break;
			case VK_SHIFT:
				kvk[VK_SHIFT].used_as_prefix = true;
				kvk[VK_LSHIFT].used_as_prefix = true;
				kvk[VK_RSHIFT].used_as_prefix = true;
				ksc[SC_LSHIFT].used_as_prefix = true;
				ksc[SC_RSHIFT].used_as_prefix = true;
				break;
			case VK_CONTROL:
				kvk[VK_CONTROL].used_as_prefix = true;
				kvk[VK_LCONTROL].used_as_prefix = true;
				kvk[VK_RCONTROL].used_as_prefix = true;
				ksc[SC_LCONTROL].used_as_prefix = true;
				ksc[SC_RCONTROL].used_as_prefix = true;
				break;
			}
			break;
...
// aBufSize is an int so that any negative values passed in from caller are not lost.
{
	...
	snprintfcat(aBuf, aBufSize, 
		...
		"of the history buffer.  Example: #KeyHistory 100  (Default is 40, Max is 500)"
